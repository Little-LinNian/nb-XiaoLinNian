...
from nonebot.plugin import on, on_message, on_startswith
...
async def mainProgram(msg,  userQQ, nickname):
    ...
    if exactMatch:
        ...
        if not result == Status.FAILURE:
            resp = MessageSegment.image(
                Path(f"{RESOURCES_BASE_PATH}/cache/{userQQ}.png").absolute().as_uri())
            return resp
        else:
            resp = MessageSegment.text("也许你今天签到过了")
            return resp
